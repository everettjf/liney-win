...
| TUI applications | vim, less, tmux, htop/btop, fzf, mc and a curses fixture on supported shells | Planned client gate |
...
| Performance | Frame-time percentiles for steady output, full-screen scroll, resize and mixed Unicode; no dropped-input gate | Startup/memory, 20k-line output and renderer frame-p95 gate; resize/mixed-Unicode scenarios planned |
...
| Area | Required evidence | Current status |
|---|---|---|
| Responsive density | Golden captures at minimum width, 100–300% DPI, long names, 1/4/16/48 panes and tab overflow | Width/DPI layout gate; golden comparison planned |
| Object semantics | Repository, ordinary folder, worktree, SSH host and Agent task have distinct icon/state/secondary text | Partial |
...
| Visual regression | Pixel/tolerance comparison of deterministic headless Direct2D captures | Capture exists; comparison gate planned |
